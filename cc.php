...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$cc_number || !$expiry || !$cvv || !$cardholder) {
    ...
    } elseif (!preg_match('/^(0[1-9]|1[0-2])\/?([0-9]{2,4})$/', $expiry)) {
        ...
        if (strpos($expiry, '/') === false) {
            $expiry = substr($expiry, 0, 2) . '/' . substr($expiry, 2);
        }
...
<main class="form-card" role="main" aria-labelledby="pageTitle">
  ...
  <form method="POST" novalidate autocomplete="off" aria-describedby="formDesc">
    ...
    <div class="mb-3">
      ...
      <input
        ...
        oninput="this.value=this.value.replace(/[^\d\s]/g,'').replace(/(.{4})/g,'$1 ').trim()"
      />
    </div>
    ...
    <div class="row g-3 mb-4">
      <div class="col-6">
        ...
        <input
          ...
          maxlength="5"
          title="Format MM/YY"
          oninput="this.value=this.value.replace(/[^\d\/]/g,'').replace(/^([2-9])$/,'0$1').replace(/^(0[1-9]|1[0-2])$/,'$1/').substring(0,5)"
        />
      </div>
      <div class="col-6">
